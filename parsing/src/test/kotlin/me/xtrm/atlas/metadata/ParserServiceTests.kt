...
import me.xtrm.atlas.metadata.api.dsl.findFor
...
internal class ParserServiceTests {
    ...
    fun `find registered target's parser`() {
        assertNotNull(
            MetadataParserService.findFor<ModMetadata>(),
    ...
    fun `throw on unknown target`() {
        assertThrows<ParserException> {
            MetadataParserService.getFor<Supplier<*>>()
        }
    }

    @Test
    fun `handle known schema version`() {
        val validSchema = MetadataParserService
            .parserRegistry[ModMetadata::class.java]
            ?.keys
            ?.first()!!
